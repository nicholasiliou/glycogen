...
export type TextMode = "off" | "fill" | "attract";
export type TextSetting = TextMode;

/** Validate an arbitrary prop value into a TextSetting (defaults to "attract"). */
export function textSettingOf(v: unknown): TextSetting {
  return v === "off" || v === "fill" || v === "attract" ? v : "attract";
}

/** Resolve the effective mode for this frame: off is used when no text field is
 *  present, otherwise the setting is used as-is. */
